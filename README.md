...
- **Cursor** — session JWT read from Cursor's `state.vscdb` (its VS Code-derived global storage), usage fetched from the endpoint Cursor's own dashboard calls. That endpoint is not published by Cursor and can change without notice, outside this project's control. Cursor and Command Code are the only providers that report what a cycle costs, so their cards can show dollars as well as a percentage.
...
- **Command Code** — API key read from `~/.commandcode/auth.json` (what `cmd login` writes) or from an exported `COMMAND_CODE_API_KEY` / `COMMANDCODE_API_KEY`, usage fetched from the `/alpha` endpoints the CLI's own `/usage` meters call. Those endpoints are not published by Command Code and can change without notice, outside this project's control. Command Code reports both of its rolling windows as dollar amounts, so its card can show the spend as well as a percentage.
